 ...
 * @Description <p>
 *     发送短信验证码或者登录等场景操作之前都需要进行图片验证码校验或者滑块验证码校验；
 *     此举是为了减少黑盒对服务端进行暴力破解密码或者频发短信轰炸请求的操作；
 *
 *        但如果滑块验证完全由前端进行操作，实际上是不能很好的进行黑盒测试的防御，很容易绕过，
 *        所以在这里推出一个前后端结合滑块验证码的实例；
 *
 * 步骤：
 * 后台生成背景图+模版抠图+抠图横坐标+抠图纵坐标；
 * 后台生成流水号，保存抠图的横坐标到内存或者redis等，返回前端背景图+模版抠图+抠图纵坐标；
 * 前端根据背景图+模版抠图+抠图纵坐标进行滑块插件展示；
 * 用户操作成功后传输流水号+滑块横坐标到后台，后台进行比较是否在误差范围内（比如5像素）返回前端显示；
 * @ClassName VerifyImageUtil
 * @create_date 2021.09.10 14:17
 * @since
 */
